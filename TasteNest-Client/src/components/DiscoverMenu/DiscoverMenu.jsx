...
const DiscoverMenu = () => {
  return (
    <div className="text-center mt-[106px]  max-w-7xl mx-auto">
      <SectionHeader title="Discover Menu" />
        ...
        {menuItems.map((item) => (
          ...
          >
            <img
              src={item.img}
              alt={item.title}
              className="w-full h-46 md:h-64 object-cover rounded-lg md:rounded-[30px]"
            />

            <div className="absolute inset-0 bg-black/10 flex flex-col items-start pl-4 md:pl-10 pt-10 rounded-[30px] md:space-y-4">
              <h3 className="text-white text-2xl md:text-3xl lg:text-5xl font-medium">
                {item.title}
              </h3>
              <p className="text-gray-100 text-base md:text-lg text-left w-4/5">
                {item.desc}
              </p>
            </div>
